  ...
  return baseArgv
    .usage('\nSimple SAML Service Provider / WS-Federation Relying Party\n\n' +
      ...
      idpCert: {
        ...
        string: true,
        coerce: (value) => {
          return removeHeaders(makeCertFileCoercer('certificate', 'IdP Public Key Signing Certificate (PEM)', KEY_CERT_HELP_TEXT)(value))
        }
      },
      idpThumbprint: {
    ...
    .then((metadata) => {
      if (metadata.protocol) {
        ...
        if (metadata.signingKeys[0]) {
          argv.idpCert = metadata.signingKeys[0];
        }
